(() => {
  ...
  var modas = {};
  function moda(name, callback) {
    modas[name] = callback;
  }
  function injectmodas(obj, el) {
    let memoizedUtilities = getUtilities(el);
    Object.entries(modas).forEach(([name, callback]) => {
...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  function normalEvaluator(el, expression) {
    let overriddenmodas = {};
    injectmodas(overriddenmodas, el);
    let dataStack = [overriddenmodas, ...closestDataStack(el)];
  ...
  moda("nextTick", () => nextTick);
  moda("dispatch", (el) => dispatch2.bind(dispatch2, el));
  moda("watch", (el, { evaluateLater: evaluateLater2, cleanup: cleanup2 }) => (key, callback) => {
  ...
  moda("store", getStores);
  moda("data", (el) => scope(el));
  moda("root", (el) => closestRoot(el));
  moda("refs", (el) => {
    if (el._x_refs_proxy)
  ...
  moda("id", (el, { cleanup: cleanup2 }) => (name, key = null) => {
  ...
  moda("el", (el) => el);
  warnMissingPluginmoda("Focus", "focus", "focus");
  warnMissingPluginmoda("Persist", "persist", "persist");
  function warnMissingPluginmoda(name, modaName, slug) {
    moda(modaName, (el) => warn(`You can't use [$${modaName}] without first installing the "${name}" plugin here: https://alpinejs.dev/plugins/${slug}`, el));
  }
  ...
  directive("data", (el, { expression }, { cleanup: cleanup2 }) => {
    ...
    expression = expression === "" ? "{}" : expression;
    let modaContext = {};
    injectmodas(modaContext, el);
    let dataProviderContext = {};
    injectDataProviders(dataProviderContext, modaContext);
    let data2 = evaluate(el, expression, { scope: dataProviderContext });
    if (data2 === void 0 || data2 === true)
      data2 = {};
    injectmodas(data2, el);
  ...
  module_default.moda("wire", (el, { cleanup: cleanup2 }) => {
    let component;
    return new Proxy({}, {
      get(target, property) {
        if (!component) {
  ...
  function history2(Alpine3) {
    Alpine3.moda("queryString", (el, { interceptor: interceptor2 }) => {
      let alias;

...
function buildPrompt(string) {
	...
	if ( prompt && chatsV2 && chatsV2.selectedTools ) {
		...
		if ( followUps.length ) {
			...
			prompt = `${modaui_localize.regarding ?? 'Regarding'} ${followUps.map(f => f.replace('entity-follow-up-', '')).join(', ')}: ${existingPrompt}`;
		}
	}
...
function renderSkillCard(str) {
	...
	'<div class="lqd-skill-card rounded-[20px] bg-card-background p-4 shadow-[0_4px_44px_hsl(0_0%_0%/5%)] md:p-5">' +
		'<button type="button" class="group mb-3 inline-flex items-center gap-1.5 text-2xs font-medium text-foreground/70 transition-colors hover:text-foreground" onclick="window.openSkillPreview(\'' + rawEncoded + '\')">' +
			...
			' ' + (modaui_localize.open_skill ?? 'Open Skill') + '</button>' +
	...
	if (responseObj._usedSkills?.length) {
		...
		if (!existing) {
			...
			responseObj._usedSkills.forEach(function(skill) {
				badge += '<span>' + (modaui_localize.used ?? 'Used') + ' ' + escapeHtml(skill.name) + ' ' + (modaui_localize.skill ?? 'Skill') + '</span>';
			});
			badge += '</div>';
...
async function handleCanvasResponseStore(responseObj) {
	try {
		...
		if (!res.ok || resData.status == 'error') {
			toastr.error(resData.message || modaui_localize.could_not_save_the_canvas_data);
		}
	} catch (error) {
		alert(error);
		console.error(error);
...
function getFrontModelEl() {
	...
	if (!chatbotFrontModel) {
		...
		defaultOption.value = '';
		defaultOption.textContent = modaui_localize.default_model || 'Default Model';
...
function cleanEntityAnnotations(text) {
	// Strip everything from :::entity-highlights (or partial :::ent) to end of text.
	// The entity block is always the last thing in the response — one simple cut.
	text = text.replace(/\s*(?:<br\s*\/?>|\n)*\s*:::[\s]*entity[- ]?highlights?[\s\S]*$/i, '');
...
function handleCouncilStreamPayload(responseObj, payload = {}) {
	...
	if (type === 'error') {
		setCouncilSummaryThinking(responseObj, false);
		setCouncilFinalSectionsVisible(responseObj, true);
		responseObj.response = [ `${payload?.message || modaui_localize.error}` ];
		finalizeCouncilBubbleAnimationState(responseObj);
		return true;
	}
...
function sendRequest(type, images, responseObj, sharedMessageUUID = null, requestOptions = {}) {
	...
	fetchEventSource('/dashboard/user/generator/generate-stream', {
		...
		onerror: err => {
			...
			responseObj.responseStreaming = false;
			responseObj.response.push(`${modaui_localize.error}: ${err.message}`);
...
async function startGenerateRequest(ev) {
	...
	if (!promptInputValue.trim()) {
		return toastr.error(modaui_localize?.please_fill_message || 'Please fill the message field',);
	}
...
function startNewDocChat(file, type) {
	...
	$.ajax({
		...
		success: function (data) {
			...
			toastr.success(modaui_localize.analyze_file_finish);
		},
		error: function (data) {
			Alpine.store('appLoadingIndicator').hide();
			$('.lqd-upload-doc-trigger').attr('disabled', false);
...
$(document).ready(function () {
		...
		if (confirm(`Are you sure you want to remove ${chatTitle}?`)) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					toastr.success(
						modaui_localize.conversation_deleted_successfully,
					);
				},
				error: function (data) {
					var err = data.responseJSON.errors;
	...
	$('#chat_sidebar_container').on('click', '.chat-item-pin', ev => {
		...
		$.ajax({
			...
			success: function (data) {
				toastr.success(isPinned ? modaui_localize.conversation_unpinned : modaui_localize.conversation_pinned);
			},
			error: (xhr, status, error) => {
				console.error('Error updating pin status:', error);
				togglePinMode();
				toastr.error(modaui_localize.conversation_pin_error);
			},
		});
	});
	...
	$('#selectDocInput').change(function () {
		if (this.files && this.files[0]) {
			...
			toastr.success(modaui_localize.analyze_file_begin);
		...
		if (!dropdown) {
			...
			dropdown.innerHTML =
				'<div class="border-b relative">' +
					...
					'<input id="slash-command-search" type="text" placeholder="'+ (modaui_localize.search_skills ?? 'Search Skills') +'" class="h-11 w-full border-0 bg-transparent py-0 pe-4 ps-9 text-heading-foreground placeholder:text-foreground/40 focus:outline-none focus:ring-0 sm:text-[12px]" />' +
				'</div>' +
				...
				'<button type="button" id="slash-manage-skills-btn" class="flex h-11 w-full items-center justify-start gap-2.5 border-t px-3 text-start text-[12px] text-foreground transition-colors hover:bg-foreground/5">' +
					...
					(modaui_localize.manage_skills ?? 'Manage Skills') +
				'</button>';

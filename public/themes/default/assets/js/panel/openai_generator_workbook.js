...
const generate = async ( message_no, creativity, maximum_length, number_of_results, prompt, openai_id, open_router_model) => {
	...
	if (stream_type == 'backend') {
		...
		try {
			...
			switch (error.status) {
				case 429:
					toastr.error(modaui_localize?.api_connection_error || 'Api Connection Error. You hit the rate limites of openai requests. Please check your Openai API Key');
					break;
				default:
					toastr.error(modaui_localize?.api_connection_error_admin || 'Api Connection Error. Please contact system administrator via Support Ticket. Error is: API Connection failed due to API keys');
			}
...
function getTinyMCEOptions(el) {
	...
	let toolbar = 'styles | modaIconRewrite | modauiButton | link | image | forecolor backcolor emoticons | bold italic underline | bullist numlist | blockquote | wordcount | alignleft aligncenter alignright | code supercode';

	if ( isGeneratorV2 ) {
		toolbar += ' | modauiEmoji modauiCover';
	}
	...
	return {
		...
		setup: function ( editor ) {
			...
			const menuItems = {
				'customwrite': {
					icon: 'modaIcon',
					text: modaui_localize?.what_would_you_like_to_do || 'What would you like to do?',
					onAction: function ( e ) {
				...
				'rewrite': {
					icon: 'modaIconRewrite',
					text: modaui_localize?.rewrite || 'Rewrite',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
				...
				'summarize': {
					icon: 'modaIconSummarize',
					text: modaui_localize?.summarize || 'Summarize',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
				...
				'makeitlonger': {
					icon: 'modaIconMakeItLonger',
					text: modaui_localize?.make_it_longer || 'Make it Longer',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
				...
				'makeitshorter': {
					icon: 'modaIconMakeItShorter',
					text: modaui_localize?.make_it_shorter || 'Make it Shorter',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
				...
				'improvewriting': {
					icon: 'modaIconImprove',
					text: modaui_localize?.improve_writing || 'Improve Writing',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
				...
				'translateto': {
					icon: 'modaIconTranslate',
					text: modaui_localize?.translate_to || 'Translate to',
					getSubmenuItems: function () {
						...
						items.push({
							type: 'menuitem',
							icon: 'modaIconSearch',
							text: modaui_localize?.search || 'Search',
				...
				'changestyle': {
					icon: 'modaIconChangeStyle',
					text: modaui_localize?.change_style_to || 'Change Style To',
					getSubmenuItems: function () {
						const styles = [
							'Professional',
							'Conversational',
				...
				'changetone': {
					icon: 'modaIconChangeTone',
					text: modaui_localize?.change_tone_to || 'Change Tone To',
					getSubmenuItems: function () {
						const tones = [
							'Formal',
							'Informal',
				...
				'simplify': {
					icon: 'modaIconSimplify',
					text: modaui_localize?.simplify || 'Simplify',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
				...
				'fixgrammaticalmistakes': {
					icon: 'modaIconFixGrammer',
					text: modaui_localize?.fix_grammatical_mistakes || 'Fix grammatical mistakes',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
...
`);
				...
				if ( emojiPickerWrapper ) {
					...
					editor.ui.registry.addButton( 'modauiEmoji', {
						text: modaui_localize?.add_emjoi ?? 'Add Emoji',
						icon: 'modaIconEmoji',
						onAction: function() {
				...
				editor.ui.registry.addButton( 'modauiCover', {
					text: modaui_localize?.add_cover ?? 'Add Cover',
					icon: 'modaIconCover',
					onAction: function() {

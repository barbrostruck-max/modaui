(() => {
	...
	function copyToClipboard(content, contentType = 'text') {
		...
		navigator.clipboard.writeText(content).then(() => {
			toastr.success(
				modaui_localize && modaui_localize[`${contentType}_content_copied_to_clipboard`] ?
					modaui_localize[`${contentType}_content_copied_to_clipboard`] :
					'Content copied to clipboard' );
		}).catch(err => {
			toastr.error( modaui_localize?.copy_failed || 'Failed to copy content' );
		});
	}

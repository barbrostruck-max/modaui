...
class Helper
{
        ...
        return Cache::remember($cacheKey, 3, static function () use ($userId) {
            return Subscription::query()
                ...
                ->whereIn('stripe_status', [
                    'active', 'trialing', 'bank_approved', 'banktransfer_approved',
                    'bank_renewed', 'free_approved', 'stripe_approved', 'paypal_approved',
                    'iyzico_approved', 'paystack_approved',
                ])
                ->first();
        });
    }

    // 【修改点 1】强制返回 true，不再校验扩展授权
    public static function showIntercomForBuyer(array $extensions = []): bool
    {
        return true; 
    }

    // 【修改点 2】强制返回 true，绕过 VIP 联网检查
    ...
    {
        return false; // 强行关闭 Demo
    }

    // 【修改点 4】强行关闭 Demo 模式，开启完整功能
    ...
    public static function generateFakeDataLastMonth(): array
    {
        ...
        $dateRange = new DatePeriod($startDate, $interval, $now->modify('+1 day'));
        $data = [];
    ...
    public static function demoDataForAdminDashboardPopularPlans(): array { return []; }
    public static function demoDataForAdminDashboardTopCountries(): false|string { return json_encode([]); }
    public static function demoDataForAdminDashboardUserTraffic(): false|string { return json_encode([]); }

    public static function checkImageDailyLimit($lockKey = 'default_lock_key'): JsonResponse
    {
        return response()->json([], 200); // 移除限制
    }
    ...
    {
        return response()->json(['status' => 'ok'], 200); // 移除限制
    }
    ...
    public static function defaultCurrency($column = 'code')
    {
        ...
        if (is_null($column)) return $currency;

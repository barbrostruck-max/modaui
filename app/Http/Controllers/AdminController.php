...
class AdminController extends Controller
{
    ...
    // 必须有构造函数初始化服务，否则 $this->service 会报错
    ...
    public function index(): View
    {
        ...
        $vip_membership = Cache::has('vip_membership') 
            ? Cache::get('vip_membership') 
            : Helper::isUserVIP();

        // 优先使用带 default. 的视图路径
        $viewPath = view()->exists('default.panel.admin.index') 
            ? 'default.panel.admin.index' 
            : 'panel.admin.index';
    ...
    {
        $mobileDevices = ['Mobile', 'Android', 'Silk/', 'Kindle', 'BlackBerry', 'Opera Mini', 'Opera Mobi'];
        foreach ($mobileDevices as $device) {
            if (str_contains($userAgent, $device)) return true;
        }
        return false;
    }
    ...
    public function users(Request $request): View
    {
        ...
        $viewPath = view()->exists('default.panel.admin.users.index') 
            ? 'default.panel.admin.users.index' 
    ...
    public function usersSearch(Request $request): View
    {
        ...
        $viewPath = view()->exists('default.panel.admin.users.components.users-table') 
            ? 'default.panel.admin.users.components.users-table' 
            : 'panel.admin.users.components.users-table';
    ...
    public function howitWorksDefaults()
    {
        ...
        $default_html = 'Want to see? <a class="text-[#FCA7FF]" href="https://mdio.shop/" target="_blank">' . __('Join') . ' moda</a>';

...
class AutoUpdateController extends Controller
{
    ...
    {
        $this->versionUrl = config('modauiupdater.version_new_base_url');
    ...
    {
        $this->updateUrl = config('modauiupdater.update_new_base_url');
    }
    ...
    private function log(string $messageGroup, string $appendMsg = '', string $type = 'info'): void
    {
        ...
        if (in_array($messageGroup, $validMessageGroups)) {
            $messageContent = __('modauiupdater.' . $messageGroup);
        } else {
            $messageContent = $messageGroup;
        }

        // Create the log message
        $message = sprintf('modaui New Updater - [%s]: %s %s', $type, $messageContent, $appendMsg);
    ...
    */
    public function update(): bool
    {
        ...
        try {
            ...
            $tmpFolderName = $this->path(config('modauiupdater.tmp_folder_name'));
    ...
    private function install($archive): bool
    {
        try {
            ...
            $updateScript = $this->path(config('modauiupdater.tmp_folder_name') . '/' . config('modauiupdater.script_filename'));

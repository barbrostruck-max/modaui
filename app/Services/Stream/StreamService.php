...
class StreamService
{
    ...
    private function emitStreamChunk(string $messageFix): void
    {
        ...
        if ($this->shouldGenerateSuggestions || $this->entityHighlightsEnabled) {
            ...
            // Detect :::entity-highlights marker — suppress all remaining output
            if ($this->entityHighlightsEnabled && str_contains($this->suggestionsBuffer, ':::') && preg_match('/:::[\s]*entity[- ]?highlights?/i', $this->suggestionsBuffer)) {
                $clean = (string) preg_replace('/\s*(<br\s*\/?>|\n)*\s*:::[\s]*entity[- ]?highlights?[\s\S]*$/si', '', $this->suggestionsBuffer);
